...
function runIntro() {
  ...
  window.addEventListener('load', () => {
    ...
    setTimeout(() => {
      ...
      // Start everything
      masterLoop(0); // kick off single rAF loop
      initTyping();
      initDataStream();
      initParticles();
      initScrollHandlers();
...
const REPORT_FILES = [
  { 
    ...
    outline: [
      ...
      { title: '5. Tạo tập tin văn bản', page: 3 },
      { title: '6. Đổi tên tập tin', page: 4 },
      { title: '7. Tạo thư mục con', page: 4 },
      { title: '8. Sao chép tập tin', page: 4 }
    ]
  },
  { 
  ...
  { 
    ...
    outline: [
      { title: 'IV. BO 7 NGUYEN TAC CA NHAN SU DUNG AI CO TRACH NHIEM', page: 3 },
      { title: 'V. INFOGRAPHIC: SU DUNG AI CO TRACH NHIEM TRONG HOC TAP', page: 4 }
    ]
  },
  { 
    ...
    outline: [
      { title: 'I. CHU DE VA CAU HOI NGHIEN CUU', page: 2 },
      { title: 'II. PHUONG PHAP TIM KIEM VA LUA CHON TAI LIEU', page: 2 },
      { title: 'III. NHAN XET TONG HOP', page: 2 },
      { title: '1. Xu huong chung', page: 2 },
      { title: '2. Diem manh va han che', page: 2 },
      { title: '3. Khoang trong nghien cuu', page: 2 },
      { title: 'IV. BANG SO SANH 5 BAI BAO KHOA HOC', page: 3 }
    ]
  },
];
            ...
            if (pdfDoc_) {
              pdfDoc_.getPage(item.page).then(function(p) {
                return p.getTextContent().then(function(tc) {
                  if (!tc.items) return;
                  var searchStr = item.title.substring(0, 15).toLowerCase().trim();
                  var matchY = -1;
